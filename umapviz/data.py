"""
Synthetic high-dimensional point clouds for UMAP.

Each sample lives in R^D (D configurable). Structure is created in a low-
dimensional latent space then rotated into D dimensions so linear PCA is blind
to the geometry UMAP should recover.

Returns (X, labels, latent) where latent is (n, 2) or (n, 3) ground truth before
the lift (same row order as X), or None if not defined (uniform). Mixed data
uses NaN rows where latent is meaningless.
"""
...
def _lift(
    ...
    """Isometry into R^dim (+ isotropic noise). X2 shape (n, 2)."""
...
def _lift3(
    ...
    """Random isometry from R^3 into R^dim (+ isotropic noise). X3 shape (n, 3)."""
...
def _normalise_latent(Z: np.ndarray, scale: float = 4.0) -> np.ndarray:
    """Scale latent (n, k) for display — same ballpark range as PCA / embedding panels."""
