...
def make_point_figure(
    ...
    fig = go.Figure(
        ...
        layout=go.Layout(
            ...
            title=dict(text=title or snap.title, x=0.5, xanchor="center",
                       font=dict(size=16, color="#18181b")),
            xaxis=dict(**axis_style, title="x₁", range=x_range),
            yaxis=dict(**axis_style, title="x₂", range=y_range, scaleanchor="x"),
...
def make_schedule_figure(schedule: NoiseSchedule) -> go.Figure:
    ...
    fig = make_subplots(
        rows=1, cols=2,
        subplot_titles=("Noise added per step (β_t)", "Signal-to-noise ratio (log scale)"),
    )
    fig.add_trace(
    ...
    fig.update_layout(
        ...
        title=dict(text=f"Noise schedule — {schedule.kind}, T={schedule.T}", x=0.5, xanchor="center",
                   font=dict(size=16, color="#18181b")),
    )
    return fig
...
def make_loss_figure(loss_history: list[float]) -> go.Figure:
    ...
    axis_style = dict(showgrid=True, gridcolor="#eef0f4", gridwidth=1,
                       zeroline=False, showline=True, linecolor="#e4e4e7", linewidth=1)
    fig = go.Figure(
        data=[go.Scatter(x=epochs, y=loss_history, mode="lines", line=dict(color=_PALETTE[5], width=2))],
        layout=go.Layout(
            font=dict(family=_FONT_FAMILY, size=13, color="#3f3f46"),
            title=dict(text="Score network training loss  E[‖ε − ε_θ(x_t, t)‖²]", x=0.5, xanchor="center",
                       font=dict(size=14, color="#18181b")),
            xaxis=dict(title="epoch", **axis_style),
            yaxis=dict(title="loss (log scale)", type="log", **axis_style),
            height=320,
