...
# ---------------------------------------------------------------------------
# Params rail — signal composition (drives the "Signal & Spectrum" tab and
# doubles as the source waveform for the spectrum-recovery demo)
# ---------------------------------------------------------------------------
with params_rail(col_params, "Signal"):
    n = st.selectbox(
...
with col_params:
    with st.expander("📖 Cooley-Tukey in a nutshell", expanded=False):
        st.markdown(
            """
...
with col_main:
    about_section(
        "The FFT is arguably the most important algorithm of the 20th "
        "century for signal processing — it turns an O(N²) computation (the "
        "naive Discrete Fourier Transform) into O(N log N), the difference "
        "between \"impossible in real time\" and \"runs on every phone doing "
        "audio/image compression, radar, medical imaging, and wireless "
        "communication.\" The **FFT vs naive DFT** tab's runtime chart is "
        "that speed-up made concrete; the **Butterfly diagram** tab shows "
        "*how*: the Cooley-Tukey formulation recursively splits the problem "
        "in half (the \"butterfly\" pattern), which is exactly the "
        "divide-and-conquer trick that produces the O(N log N) scaling.",
        [
