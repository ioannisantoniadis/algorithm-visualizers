...
# ---------------------------------------------------------------------------
# Params rail — noise schedule
# ---------------------------------------------------------------------------
with params_rail(col_params, "Noise schedule"):
    T = st.slider(
...
# ---------------------------------------------------------------------------
# Params rail — score network
# ---------------------------------------------------------------------------
with params_rail(col_params, "Score network"):
...
with col_main:
    about_section(
        "Diffusion models are the technology behind most modern image "
        "generators — Stable Diffusion, DALL·E, Midjourney — having replaced "
        "GANs as the dominant generative approach because they're far more "
        "stable to train and produce higher-fidelity, more diverse samples. "
        "The core idea is almost deceptively simple: train a network to "
        "reverse a fixed noising process one small step at a time. The "
        "**Forward** / **Reverse** toggle above lets you watch both halves of "
        "that idea directly on a 2-D toy dataset — forward is a fixed, "
        "untrained process that always looks the same; reverse is the "
        "*trained* score network's attempt to invert it, starting from pure "
        "noise.",
        [
            ...
            "(2015). [\"Deep Unsupervised Learning using Nonequilibrium "
            "Thermodynamics.\"](https://arxiv.org/abs/1503.03585) "
            "arXiv:1503.03585 — the original diffusion probabilistic model "
