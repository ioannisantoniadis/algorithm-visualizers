...
Each generator returns (true_pos, observations) where:
  ...
  observations  — float64 array (n_steps, 2): true_pos + measurement
                  noise. This is the only thing the Kalman filter is
                  given.

A small amount of "real world" process noise is baked into the true
motion itself (independent of the filter's assumed Q). This means the
constant-velocity model the filter uses is always a simplification of
reality — exactly like a real tracking problem — which is what makes the
process-noise slider in the app meaningful: a Q that's too small makes
the filter lag during turns, a Q that's too large makes it jittery.
...
# filter's assumed process noise Q. A real target is never perfectly linear.
...
def make_linear(
    ...
    """Near-constant-velocity motion in a random direction.

    This is the scenario the constant-velocity Kalman model was designed
    for — with a well-tuned Q, the filter should track this almost
    perfectly and smooth out most of the measurement noise.
    """
...
def make_circular(
    ...
    """Orbiting motion at (roughly) constant angular velocity.

    A constant-velocity model has no notion of curvature, so the filter
    is always chasing a target that's constantly "turning". A larger Q
    lets it keep up; a small Q makes it cut corners on the inside of
    the curve.
    """
    rng = np.random.default_rng(seed)
    radius = 8.0
...
def make_weave(
    ...
    Exercises the filter's ability to track periodic accelerations —
    a stress test between the smooth-but-laggy and jittery-but-responsive
    extremes of the Q trade-off.
    """
    rng = np.random.default_rng(seed)
...
def make_maneuver(
    ...
    The sharpest test of Q: at each turn the true velocity jumps
    discontinuously, which a constant-velocity model can only explain
    away as "process noise". Watch the uncertainty ellipse balloon right
    after each turn and the estimate visibly lag before catching up.
    """
    rng = np.random.default_rng(seed)
    speed = 1.4
...
def make_trajectory(
    ...
    Parameters
    ----------
    kind:               one of TRAJ_KEYS
    n_steps:            number of time steps to simulate
    dt:                 time between steps
    seed:               random seed (controls both true-motion jitter and
                         measurement noise draws)
    measurement_noise:  std-dev of the Gaussian sensor noise added to
                         the true position to produce each observation
    """
    if kind == "linear":
